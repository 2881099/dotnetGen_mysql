...
namespace CSRedis {
	public partial class QuickHelperBase {
		...
		/// <returns></returns>
		public static T Cache<T>(string key, int timeoutSeconds, Func<T> getData, Func<T, string> serialize = null, Func<string, T> deserialize = null) {
			...
			var ret = getData();
			Set(key, serialize == null ? Newtonsoft.Json.JsonConvert.SerializeObject(ret) : serialize(ret), timeoutSeconds);
			return ret;
		}
		/// <summary>
		/// 缓存壳(哈希表)
		...
		/// <param name="deserialize">反序列化函数，默认：Newtonsoft.Json.JsonConvert.DeserializeObject&lt;<typeparamref name="T"/>&gt;</param>
		/// <returns></returns>
		public static T Cache<T>(string key, string field, long timeoutSeconds, Func<T> getData, Func<T, string> serialize = null, Func<string, T> deserialize = null) {
			if (timeoutSeconds <= 0) return getData();
			...
			var ret = getData();
			HashSet(key, field, serialize == null ? Newtonsoft.Json.JsonConvert.SerializeObject((ret, DateTime.Now)) : Newtonsoft.Json.JsonConvert.SerializeObject((serialize(ret), DateTime.Now)));
			return ret;
		}
		/// <summary>
		/// 设置指定 key 的值
